...
// ─── Brand tokens (DS v3.0 — STRYVR client) ──────────────────────────────────
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Arial,sans-serif;background:#0d0d0d;margin:0;padding:40px 16px;color-scheme:dark;">
      ...
      <table style="width:100%;border-collapse:collapse;">
        <tr>
          <td style="vertical-align:middle;">
            <img src="${SITE_URL}/logo/Logo%20STRYVR%20(silver).png" alt="STRYVR" width="32" height="32" style="display:inline-block;vertical-align:middle;margin-right:8px;" />
...
function ctaButton(href: string, label: string): string {
  return `<a href="${href}"
    style="display:inline-block;background:${DS.accent};color:${DS.accentText};text-decoration:none;
           font-weight:800;font-size:14px;padding:13px 28px;border-radius:10px;
...
export async function sendBilanEmail(params: SendBilanEmailParams) {
  const { to, clientFirstName, coachName, templateName, bilanUrl, expiresAt } =
    params;
...
export async function sendBilanCompletedEmail(
  params: SendBilanCompletedEmailParams,
) {
  const { to, coachFirstName, clientFullName, templateName, dashboardUrl } =
    params;
...
export async function sendPaymentReminderEmail(
  ...
  if (paymentMethod)
    rows.push({
      label: "Méthode habituelle",
      value: METHOD_LABELS[paymentMethod] ?? paymentMethod,
    });

  await sendMail({
    from: fromName
      ? `${fromName} <${process.env.RESEND_FROM_EMAIL || "onboarding@resend.dev"}>`
      : FROM,
    to,
...
export async function sendReactivationEmail(
  params: SendReactivationEmailParams,
) {
...
export async function sendBetaWaitlistEmail(
  params: SendBetaWaitlistEmailParams,
) {
  ...
  const intro = isCoach
    ? `Tu es inscrit en tant que <strong style="color:${DS.white};">Coach</strong> sur la liste bêta STRYVR. On te contacte en premier au lancement pour accéder à la plateforme de génération de programmes et de suivi physiologique de tes clients.`
  ...
  const whatToExpect = isCoach
    ...
    : `<div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:10px;padding:20px 24px;margin-bottom:24px;">
        ...
        <p style="font-size:13px;color:rgba(255,255,255,0.55);line-height:1.65;margin:0;">✦ Ton retour construit la prochaine version de l'app</p>
      </div>`;

  await sendMail({
    from: FROM,
    to,
    subject: `${firstName} — tu es sur la liste bêta STRYVR`,
    html: emailTemplate({
      body: `
        ${greeting(firstName)}
        ${bodyText(intro)}
        ${whatToExpect}
        ${infoTable([
          { label: "Profil", value: roleLabel, accent: true },
          { label: "Statut", value: "Liste bêta — confirmé" },
        ])}
        ${hint("On te contacte dès l'ouverture de l'accès. Zéro spam. Désabonnement en 1 clic.")}
      `,
    }),
  });
}
...
export async function sendWelcomeEmail(params: SendWelcomeEmailParams) {
  ...
  const intro = coachName
    ? `Votre mot de passe a bien été créé. Bienvenue sur STRYVRR — votre espace personnel configuré par <strong style="color:${DS.white};">${coachName}</strong> est maintenant accessible.`
    : `Votre mot de passe a bien été créé. Bienvenue sur STRYVRR — votre espace personnel est maintenant accessible.`;

  await sendMail({
    from: FROM,
    to,
    subject: "Bienvenue sur STRYVRR — ton accès est actif",
    html: emailTemplate({
      senderLabel: coachName ?? undefined,
      body: `
        ${greeting(clientFirstName)}
